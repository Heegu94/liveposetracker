...
#             print('=========')
            ...
            boxes_xywh = [[x1,y1,x2-x1,y2-y1] for [x1,y1,x2,y2] in boxes if int(y2-y1) > int((x2-x1)*1.5)] #x2-x1 >30 & y2-y1 <600   
            ...
            for track in self.tracker.tracks:
                ...
                person_data["person_id"] = track.track_id
                person_data["bbox"]= [int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])]
                data.append(person_data)
                #print(data)
                     
            people_data["people"] = data
            people_data["frame_id"] = self.cnt
            #print(json.dumps(people_data, ensure_ascii=False, indent="\t"))

            with open('D:/output-id/walk_tour-edit-%09d_bbox.json' %(self.cnt),'w', encoding='utf-8') as make_file:
                json.dump(people_data, make_file, ensure_ascii=False, indent="\t")
